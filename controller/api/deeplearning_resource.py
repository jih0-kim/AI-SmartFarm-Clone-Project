...
def deeplearning_model_load():
    ...
    # 현재 텐서플로의 세션을 get
    tf_sess = tf.compat.v1.keras.backend.get_session()
    # 현재 텐서플로 세션의 그래프를 저장
    graph = tf_sess.graph
    ...
    with graph.as_default():  # 저장한 그래프와 연결
        ...
        # 딥러닝 모델 로드
        deeplearnig_model_instance = tf.keras.models.load_model(f'static/kerasmodel/{deep_learning_config["model_file_name"]}',
                                                compile=False, custom_objects={'FixedDropout':FixedDropout(rate=0.2)})  # 모델을 로드하여 연결된 그래프에서 불러 저장
...
def inference_one_image():
    ...
    # 리사이즈 값 - 딥러닝 모델 학습 시 설정한 딥러닝모델입력부분 사이즈 -> input_shape=(128, 128, 3) 와 동일 해야 함
    RESIZE_W = 200
    RESIZE_H = 200
    ...
    img = tf.keras.preprocessing.image.load_img(original_directory, target_size=(RESIZE_H, RESIZE_W))
    ...
    with tf_sess.as_default(), graph.as_default():
        ...
        grads = tf.keras.backend.gradients(y_c, last_conv_layer)[0]
        gradient_function = tf.keras.backend.function([deeplearnig_model_instance.input], [last_conv_layer, grads])
    ...
    if deep_learning_config['task_type'] == 'binary':
        ...
        if update_setting_1.setting_value == "True":
            ...
            if pred[0] >= db_mail_th:
                ...
                # 보내는메일주소, 받는메일주소, 메일내용
                sendMail('smartfarm.dodo@gmail.com', 'smartfarm.dodo@gmail.com', content)
